...
export async function analyzeProjectWithAI(content: string, filename: string): Promise<AIProjectAnalysis> {
...
export async function suggestTasks(project: Project): Promise<string[]> {
  const prompt = `프로젝트 정보:
- 이름: ${project.name}
- 설명: ${project.description || "없음"}
- 스택: ${(project.stack || []).join(", ") || "미정"}
- 상태: ${project.status}
- 완료된 태스크: ${project.tasks?.filter(t => t.done).map(t => t.text).join(", ") || "없음"}
...
export async function generateDescription(project: Project): Promise<string> {
