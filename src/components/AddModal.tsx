...
export default function AddModal({ onAdd, onClose }: AddModalProps) {
  ...
  const [room,        setRoom]        = useState<RoomKey>("lab");
  ...
  const updateTask = (i: number, val: string) => setTasks(t => t.map((x, idx) => idx === i ? { ...x, text: val } : x));
  const removeTask = (i: number) => setTasks(t => t.filter((_, idx) => idx !== i));

  const submit = () => {
    if (!name.trim()) return;
    const stack = stackInput.split(",").map(s => s.trim()).filter(Boolean);
    const finalTasks: Task[] = tasks.filter(t => t.text.trim()).map((t, i) => ({ id: `t${Date.now()}${i}`, text: t.text.trim(), done: t.done }));
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.9)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", overflowY: "auto", padding: "20px 0" }}
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: 350, background: "#0e0e12", border: "2px solid #facc1533", padding: 16, display: "flex", flexDirection: "column", gap: 9 }}>
        ...
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="프로젝트 이름" maxLength={60} style={inp} autoFocus onKeyDown={e => e.key === "Enter" && submit()} />
        </div>

        <div>
          <div style={lbl}>DESCRIPTION</div>
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="프로젝트 한 줄 소개..." rows={2} maxLength={200}
        ...
        <div>
          ...
          <input value={stackInput} onChange={e => setStackInput(e.target.value)} placeholder="React, Node, Python..." maxLength={100} style={inp} />
        </div>
