export const daysSince = (d) => Math.floor((Date.now() - new Date(d)) / 864e5);

export const neglect = (d, s) => {
...
export function analyzeFile(content, filename = "") {
  ...
  try {
    ...
    if (d.name && (d.scripts !== undefined || d.dependencies !== undefined || d.devDependencies !== undefined)) {
      ...
      return {
        ...
        room: stack.includes("Next.js") || stack.includes("React") || stack.includes("Vue") ? "office" : stack.includes("DB") || stack.includes("Supabase") ? "server" : "lab",
      };
    }
  } catch (e) { /* not json */ }
  ...
  if (fname.includes("readme") || fname.endsWith(".md") || content.trimStart().startsWith("#")) {
    ...
    const title = lines.find(l => l.startsWith("# "))?.replace(/^#\s+/, "").trim() || filename.replace(/\.md$/i, "") || "새 프로젝트";
    const h2Sections = lines.filter(l => l.startsWith("## ")).map(l => l.replace(/^##\s+/, "").trim());
    const tasks = h2Sections.slice(0, 8).map((s, i) => ({
      id: `t${ts}${i}`, text: s, done: false,
    }));
    if (tasks.length === 0) {
      tasks.push(
    ...
    return {
      ...
      stack: [],
      tasks,
      detected: `README 분석 — ${h2Sections.length}개 섹션 감지`,
      room: "lab",
    };
  }
  ...
  if (fname.includes(".env")) {
    ...
    return {
      name: filename || "환경변수 설정",
      stack: [],
      tasks,
      detected: `.env 파일 — ${keys.length}개 키 감지`,
      room: "server",
    };
  }
...
export function parseFile(content) {
  const r = analyzeFile(content, "package.json");
  return r ? {
    id: Date.now(),
    name: r.name,
    status: "active",
