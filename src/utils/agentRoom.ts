...
// 방 배정 키워드 패턴 — 모듈 로드 시 1회만 컴파일
const RE_SERVER  = /서버|배포|헬스체크|응답|장애|복구|ping|uptime/;
const RE_MEETING = /회의|보고|검토|리뷰|스프린트|미팅/;
const RE_LOUNGE  = /커피|휴식|점심|잡담/;
const RE_STORAGE = /문서|백업|로그|테스트/;
...
export function pickRoom(agent: Pick<AgentState, "rank">, task: string, assignedRoom?: RoomKey): RoomKey {
  ...
  if (RE_SERVER.test(t))  return "server";
  if (RE_MEETING.test(t)) return "meeting";
  if (RE_LOUNGE.test(t))  return "lounge";
  if (RE_STORAGE.test(t)) return "storage";
